#
# ============================================================================
# COCKPIT DÉCISIONNEL BOURSIER v5.5 --- "DATA-DRIVEN PORTFOLIO MANAGEMENT SYSTEM"
# Lead Dev: Claude (Anthropic)
# v5.4 → v5.5 :
#   • ETF_LIBRARY : référentiel central unique (métadonnées, catégories, targets)
#   • PortfolioConfigManager : persistance JSON découplée (portfolio_positions.json)
#   • TransactionEngine : reconstruction dynamique depuis transactions.json
#   • Moteurs génériques : PortfolioEngine & QuantRiskEngine agnostiques des tickers
#   • Onglet Transactions : enregistrement BUY/SELL dans l'UI
#   • Sidebar ETF_LIBRARY : ajout de nouvelles positions depuis le référentiel
#   • Toute la logique v5.4 préservée (MWR, gold fallback, CSS, pédagogie)
#
...
st.set_page_config(
    page_title="Cockpit v5.5 · PMS Institutionnel",
    page_icon="🎯",
    layout="wide",
...
# ── v5.5 : ETF_LIBRARY --- Référentiel Central (unique source de vérité) ───────

ETF_LIBRARY: Dict[str, Dict] = {
    "MWRD": {
        "nom": "MSCI World AV", "name": "Amundi MSCI World UCITS DR USD",
        "yf": "MWRD.PA", "yf_fallbacks": ["IWDA.AS", "EUNL.DE"],
        "isin": "LU1437016972", "category": "Core", "asset_class": "Equity",
        "region": "World", "theme": "World", "currency": "EUR",
        "benchmark": "MSCI World", "risk_type": "Core",
        "hedged": False, "max_weight": 1.00, "volatility_target": 0.16,
        "enveloppe": "AV", "initial_target": None,
    },
    "DCAM": {
        "nom": "MSCI World PEA", "name": "Amundi MSCI World UCITS PEA",
        "yf": "DCAM.PA", "yf_fallbacks": [],
        "isin": "FR0010315770", "category": "Core", "asset_class": "Equity",
        "region": "World", "theme": "World", "currency": "EUR",
        "benchmark": "MSCI World", "risk_type": "Core",
        "hedged": False, "max_weight": 1.00, "volatility_target": 0.16,
        "enveloppe": "PEA", "initial_target": None,
    },
    "ANRJ": {
        "nom": "Global Hydrogen", "name": "Amundi Global Hydrogen",
        "yf": "ANRJ.PA", "yf_fallbacks": [],
        "isin": "FR0010930644", "category": "Satellite", "asset_class": "Thematic",
        "region": "Global", "theme": "Hydrogen", "currency": "EUR",
        "benchmark": "Clean Energy", "risk_type": "HighVol",
        "hedged": False, "max_weight": 0.35, "volatility_target": 0.35,
        "enveloppe": "AV", "initial_target": 0.25,
    },
    "AASI": {
        "nom": "EM Asia", "name": "Amundi MSCI EM Asia",
        "yf": "AASI.PA", "yf_fallbacks": [],
        "isin": "LU1681045370", "category": "Satellite", "asset_class": "Equity",
        "region": "Asia EM", "theme": "EM Asia", "currency": "EUR",
        "benchmark": "MSCI EM Asia", "risk_type": "HighVol",
        "hedged": False, "max_weight": 0.35, "volatility_target": 0.30,
        "enveloppe": "AV", "initial_target": 0.25,
    },
    "XGDE": {
        "nom": "Or Physique", "name": "Xtrackers Physical Gold EUR Hedged",
        "yf": "OR-EUR.PA", "yf_fallbacks": ["DE000SLA8RU8.SG", "CGLD.PA", "GOLD.PA"],
        "isin": "DE000A1EK0G3", "category": "Hedge", "asset_class": "Commodity",
        "region": "Global", "theme": "Gold", "currency": "EUR",
        "benchmark": "Gold", "risk_type": "Defensive",
        "hedged": True, "max_weight": 0.20, "volatility_target": 0.15,
        "enveloppe": "AV", "initial_target": None,
    },
}

# ── v5.5 : Dériver INITIAL_TARGETS depuis ETF_LIBRARY ────────────────────────
INITIAL_TARGETS: Dict[str, float] = {
    meta["nom"]: meta["initial_target"]
    for meta in ETF_LIBRARY.values()
    if meta["initial_target"] is not None
}

# ── v5.5 : POSITIONS_BASE --- Calculée dynamiquement depuis ETF_LIBRARY ────────
def _build_positions_base_from_library() -> List[Dict]:
    """Construit la liste POSITIONS_BASE à partir de ETF_LIBRARY (fallback uniquement)."""
    return [
        {
            "nom": meta["nom"],
...
def enrich_positions(raw_positions: List[Dict]) -> List[Dict]:
    """
    v5.5 : Convertit le format JSON compact (ticker interne + parts + prm)
    vers le format étendu utilisé par PortfolioEngine.compute_portfolio().
    Format entrée : [{"ticker": "MWRD", "parts": 41.82, "prm": 132.67, "account": "AV"}]
    Format sortie : [{"nom": "...", "tickers": [...], "parts": ..., "prm": ..., "enveloppe": "..."}]
    """
    result = []
    for pos in raw_positions:
        ...
        result.append({
            ...
            "_tk_id": tk_id,  # conservation de la clé interne
        })
    return result

# v5.4 : tickers fallback Or pour QuantRiskEngine (inchangé)
...
def _cached_live_prices() -> Dict[str, Dict]:
    # v5.5 : Dériver les tickers depuis ETF_LIBRARY (plus de POSITIONS_BASE)
    all_tickers = []
    for meta in ETF_LIBRARY.values():
        all_tickers.append(meta["yf"])
        all_tickers.extend(meta.get("yf_fallbacks", []))
    all_tickers.extend(list(MACRO_TICKERS.keys()))
    all_tickers.extend(REGIME_TICKERS)
    all_tickers.extend(PROXIES_ANRJ + PROXIES_AASI)
    all_tickers.extend(GOLD_TICKERS_FALLBACK)
...
# ── v5.5 : PortfolioConfigManager ────────────────────────────────────────
class PortfolioConfigManager:
    """Gestionnaire de positions basé sur portfolio_positions.json (v5.5)."""
    def __init__(self, file_path: str = _PORTFOLIO_JSON):
        self.file_path = file_path

    def load_positions(self) -> List[Dict]:
        """Charge les positions depuis JSON. Retourne le fallback ETF_LIBRARY si absent."""
        try:
            ...
            pass
        # Fallback : positions initiales depuis ETF_LIBRARY
        return [
            {"ticker": "MWRD", "parts": 36.33, "prm": 140.41, "account": "AV"},
            {"ticker": "DCAM", "parts": 481.0, "prm": 5.5937, "account": "PEA"},
            {"ticker": "ANRJ", "parts": 4.7701, "prm": 707.55, "account": "AV"},
            {"ticker": "AASI", "parts": 40.8272, "prm": 49.96, "account": "AV"},
            {"ticker": "XGDE", "parts": 4.5902, "prm": 163.39, "account": "AV"},
        ]

    def save_positions(self, positions: List[Dict]) -> bool:
        """Sauvegarde les positions au format compact dans portfolio_positions.json."""
        try:
...
# ── v5.5 : TransactionEngine ──────────────────────────────────────────────
class TransactionEngine:
    """Reconstruction dynamique du portefeuille à partir de transactions.json (v5.5)."""
    ...
    def rebuild_portfolio_at_date(self, target_date: str = None) -> Dict[str, Dict]:
        """Reconstruit la composition du portefeuille jusqu'à target_date."""
        txs = self.load_transactions()
    ...
    def get_portfolio_as_positions(self) -> List[Dict]:
        """Reconstruit les positions depuis le livre de comptes."""
...
class PortfolioEngine:
    ...
    def compute_target_weight(self, nom: str, ticker: str,
        ...
        # v5.5 : chercher initial_target dans ETF_LIBRARY si disponible
        it = INITIAL_TARGETS.get(nom, None)
        if it is None:
            lib_entry = next((m for m in ETF_LIBRARY.values() if m["nom"] == nom), None)
            it = lib_entry["initial_target"] if lib_entry and lib_entry["initial_target"] else 0.25
    ...
    def check_leadership_alerts(self) -> List[Dict]:
        ...
        if world_close is None:
            return alerts
        # v5.5 : Boucle générique via ETF_LIBRARY (catégorie Satellite)
        satellite_pairs = [
            (meta["nom"], meta["yf"])
            for meta in ETF_LIBRARY.values()
            if meta["category"] == "Satellite"
        ]
        for nom, tk in satellite_pairs:
...
# ─────────────────────────────────────────────────────────────────────────────
# MODULE 13 : STREAMLIT UI --- v5.5 : ETF_LIBRARY sidebar + onglet Transactions
# ─────────────────────────────────────────────────────────────────────────────
...
class StreamlitUI:
    ...
    def render_sidebar(self) -> Tuple[bool, List[Dict], float, float, float]:
        st.sidebar.markdown("## ⚙️ Paramètres v5.5")
...
# ── v5.5 : Section DCA --- Édition directe via ETF_LIBRARY ──────────────
with st.sidebar.expander("⚙️ Configuration des Positions (ETF_LIBRARY)", expanded=False):
    ...
    for pos in raw_pos:
        ...
        n_parts = c1.number_input(
            "Parts", value=float(pos.get("parts", 0)),
            key=f"dca_parts_{tk_id}", format="%.4f", step=0.0001,
        )
        n_prm = c2.number_input(
            "PRM (€)", value=float(pos.get("prm", 0)),
            key=f"dca_prm_{tk_id}", format="%.4f", step=0.01,
        )
    ...
    if available:
        ...
        if chosen != "(choisir)" and st.button("➕ Ajouter", key="sidebar_add_btn"):
            ...
            new_raw.append({"ticker": chosen, "parts": 0.0, "prm": 0.0, "account": meta["enveloppe"]})
...
def render_header(self, mode_direct: bool, live_ok: int, live_total: int):
    ...
    st.markdown(
        ...
        '<span style="font-family:Space Mono,monospace;font-size:.9rem;color:#6B7585;">v5.5 · PÉDAGOGIQUE</span></div>',
        unsafe_allow_html=True)
...
def render_regime_banner(self, regime: Dict):
    ...
    st.markdown(
        ...
        f'<div style="font-family:Space Mono,monospace;font-size:1.2rem;">Score : <b>{sc:+d}/5</b></div>'
    ...
    with st.expander("ℹ️ Comment lire la météo des marchés ?", expanded=False):
        ...
        with col_exp:
            ...
            st.markdown(
                ...
                f'💡 <b>Que faire ?</b><br>{reg_trans["action"]}</div></div>', unsafe_allow_html=True)
        with col_comp:
            ...
            for i, comp in enumerate(regime.get("components", [])):
                with comp_cols[i % 5]:
                    ...
                    st.markdown(
                        ...
                        f'color:{"#22C55E" if bull else "#FF3131" if bull is False else "#6B7585"};">{sc_}</div>'
                        f'<div style="font-size:.68rem;color:#4B5563;margin-top:.2rem;">{comp["val"]}</div></div>',
                        unsafe_allow_html=True)
...
def render_command_center(self, ptf: Dict, bench: Dict, mode_direct: bool, pm: PersistenceManager):
    ...
    with c1:
        ...
        st.markdown(
            ...
            f'{s(vj)}{vj:,.2f}€ ({s(vjp)}{vjp:.2f}%) vs hier</div></div>', unsafe_allow_html=True)
    with c2:
        gr = ptf["gain_reel"]
    ...
    with c4:
        ...
        st.markdown(
            ...
            f'<div class="kpi-label">MSCI World MWR<span class="mwr-badge">AJUSTÉ</span><span class="live-badge">LIVE</span></div>'
    ...
    if mwr_adj is not None:
        ...
        st.markdown(
            f'<div class="pedagogy-box">'
            f'<div class="pedagogy-title">🆕 v5.5 --- Benchmark MWR Cash-Flow Adjusted</div>'
    ...
    with col_eq:
        ...
        if fig_eq:
            ...
            st.markdown(
                ...
                'Les zones colorées indiquent le régime de marché pendant chaque période.</div>',
                unsafe_allow_html=True)
        else:
            st.markdown(
...
def render_leadership_comparison(self, nom: str, ticker: str, color_sat: str = "#D4AF37"):
    ...
    with st.container():
        st.markdown(
            ...
            'dans votre portefeuille --- autant acheter du MSCI World pur et simple.</div></div>',
            unsafe_allow_html=True)
    ...
    if labels and sat_perfs and world_perfs:
        ...
        with col_verdict:
            ...
            st.markdown(
                ...
                f'<div style="font-size:.85rem;color:#CBD5E1;margin-top:.5rem;">💡 {verdict["action"]}</div></div>',
                unsafe_allow_html=True)
    else:
    ...
    for i, (tk, name, color, custom_df) in enumerate(risk_assets):
        with cols[i]:
            ...
            st.markdown(f'<div class="card" style="border-top:3px solid {color};">'
                        f'<div class="kpi-label">{name}</div>{gold_note}', unsafe_allow_html=True)
            ...
            st.markdown(
                ...
                f'<div class="pedago-metric-explain">{vol_t["explain"].split(chr(10))[0]}</div></div>',
                unsafe_allow_html=True)
            ...
            st.markdown(
                ...
                f'<div class="pedago-metric-explain">{beta_explain_short}</div></div>',
                unsafe_allow_html=True)
            ...
            st.markdown(
                ...
                f'<div class="pedago-metric-explain">{dd_t["explain"][:80]}...</div></div>',
                unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col_corr:
            ...
            if corr_df is not None:
                ...
                st.markdown(
                    ...
                    'Idéalement, vos actifs ne devraient pas tous monter et baisser en même temps.</div>',
                    unsafe_allow_html=True)
            else:
...
def render_satellite_card_pedagogic(self, nom: str, ticker: str,
    ...
    with c_score:
        st.markdown(
            ...
            f'<div style="text-align:center;font-weight:700;color:#E2E8F0;font-size:.95rem;">{simple_score["label"]}</div>'
            f'<div style="text-align:center;font-size:.82rem;color:#8892AA;margin-top:.3rem;">{simple_score["explain"]}</div></div>',
            unsafe_allow_html=True)
    with c_action:
        ...
        for detail in strat_full["details"]:
            ...
            st.markdown(
                f'<div style="display:flex;align-items:flex-start;gap:.5rem;margin:.3rem 0;font-size:.84rem;">'
                ...
                f'<br><span style="color:#8892AA;font-size:.78rem;">{detail["label"]}</span></div></div>',
                unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="{strat_full["verdict_cls"]} verdict-card">'
                f'{strat_full["verdict"]}'
                f'<div style="font-size:.82rem;margin-top:.4rem;opacity:.9;">💡 {simple_score["action"]}</div></div>',
                unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'<span style="font-size:.84rem;color:#CBD5E1;">💡 {sent_verdict["action"]}</span></div>',
        unsafe_allow_html=True)
        ...
        with col_detail:
            ...
            if action == "RÉDUIRE":
                st.markdown(
                    ...
                    f'Votre position est trop importante ({cur_pct:.1f}%) par rapport à la cible ({tgt_pct:.1f}%).</div></div>',
                    unsafe_allow_html=True)
                ...
                st.markdown(
                    ...
                    f'<div style="font-size:.82rem;color:#8892AA;margin-top:.3rem;">'
                    f'Vous êtes en dessous de la cible ({cur_pct:.1f}% vs {tgt_pct:.1f}%).</div></div>',
                    unsafe_allow_html=True)
            else:
                st.markdown(
                    ...
                    f'<div style="font-size:.82rem;color:#8892AA;margin-top:.3rem;">'
                    f'Position à {cur_pct:.1f}% --- objectif {tgt_pct:.1f}%.</div></div>',
                    unsafe_allow_html=True)
        st.markdown(
...
def render_sentinelles_macro(self, ptf: Dict):
    ...
    with col_s:
        ...
        st.markdown(
            ...
            f'<div style="background:{bc};width:{min(poids_s,100):.1f}%;height:8px;border-radius:6px;"></div></div>',
            unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with col_m:
...
def render_fiscal_simulator(self, ptf: Dict):
    ...
    if avert_sim:
        ...
        st.markdown(
            ...
            f'<div><div class="kpi-label">Vous recevez</div><div class="kpi-value" style="color:#22C55E;">{net_sim:,.2f}€</div></div></div>',
            unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# ── v5.5 : ONGLET TRANSACTIONS ───────────────────────────────────────────
def render_transactions_tab(self):
    ...
    with st.expander("➕ Enregistrer un nouvel ordre", expanded=True):
        ...
        with c2:
            tk_options = list(ETF_LIBRARY.keys())
            tx_ticker = st.selectbox("ETF", tk_options, key="tx_ticker")
        with c3:
            ...
            if st.button("✅ Enregistrer l'ordre", type="primary", use_container_width=True):
                if tx_parts > 0 and tx_price > 0:
                    tx_record = {
                        ...
                        "parts": tx_parts,
                        "price": tx_price,
                        "montant": round(tx_parts * tx_price, 2),
                        "note": tx_note,
                    }
        ...
        with col_info:
            if tx_parts > 0 and tx_price > 0:
                ...
                st.markdown(
                    ...
                    f'Montant total : <b>{montant:,.2f}€</b> | '
                    f'ETF : {ETF_LIBRARY.get(tx_ticker, {}).get("nom", tx_ticker)} | '
                    f'Enveloppe : {ETF_LIBRARY.get(tx_ticker, {}).get("enveloppe", "N/A")}</div>',
                    unsafe_allow_html=True)
    ...
    for tx in sorted(txs, key=lambda x: x.get("date",""), reverse=True):
        ...
        rows.append({
            ...
            "Parts": f"{tx.get('parts', 0):.4f}",
            "Prix": f"{tx.get('price', 0):.4f}€",
            "Montant": f"{tx.get('montant', 0):,.2f}€",
            "Note": tx.get("note", ""),
        })
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    ...
    if rebuilt:
        ...
        for tk_id, data in rebuilt.items():
            ...
            r_rows.append({
                "Ticker": tk_id,
                "Nom": meta.get("nom", tk_id),
                "Parts": f"{data['parts']:.4f}",
                "PRMin (€)": f"{prm:.4f}",
                "Investi (€)": f"{data['total_cost']:,.2f}",
                "Enveloppe": meta.get("enveloppe","?"),
            })
...
def render_footer(self, mode_direct: bool, capital: float,
    ...
    with col_f1:
        ...
        st.caption(
            f"◈ Cockpit v5.5 Pédagogique · {mode_txt} · "
...
# ─────────────────────────────────────────────────────────────────────────────
# MODULE 14 : POINT D'ENTRÉE PRINCIPAL --- v5.5 : ETF_LIBRARY + Tabs + TransactionEngine
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    # v5.5 : Chargement depuis PortfolioConfigManager (portfolio_positions.json)
    ...
    # ── Guide pédagogique ─────────────────────────────────────────────────────
    with st.expander("📚 Guide d'utilisation du Cockpit v5.5", expanded=False):
        st.markdown("""
        **Bienvenue dans votre Cockpit PMS Institutionnel v5.5**
        ...
        **v5.5 --- Nouveautés :**
        - 🏗️ **ETF_LIBRARY** : référentiel central --- ajoutez un ETF une seule fois, partout disponible
        - 📈 **Onglet Transactions** : enregistrez BUY/SELL → reconstruction automatique du portefeuille
        - 💾 **portfolio_positions.json** : fichier dédié aux positions, indépendant de la config
        - 🔄 **TransactionEngine** : recalcul du portefeuille à n'importe quelle date

        **Règle d'or :** N'agissez que si PLUSIEURS indicateurs pointent dans la même direction.
        """)

    # ── Rendu UI ──────────────────────────────────────────────────────────────
    tab_dashboard, tab_transactions = st.tabs(["📊 Dashboard", "📈 Transactions"])
    ...
    with tab_dashboard:
        ...
        for al in ld_alerts:
            ...
            st.markdown(
                ...
                f'<span style="font-size:.85rem;">→ Vérifiez la section Leadership ci-dessous.</span></div>',
                unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            f'<b>🔥 Hydrogen (ANRJ.PA) --- Alerte décisionnelle</b><br>{h_msg}</div>',
            unsafe_allow_html=True)
        with st.container():
        ...
        st.markdown(
            ...
            f'<b>🌏 EM Asia (AASI.PA) --- Alerte décisionnelle</b><br>{a_msg}</div>',
            unsafe_allow_html=True)
        with st.container():
