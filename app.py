...
class StreamlitUI:
    ...
    def render_sidebar(self) -> Tuple[bool, List[Dict], float, float, float]:
        ...
        with st.sidebar.expander("⚙️ Configuration des Positions (ETF_LIBRARY)", expanded=False):
            ...
            # Ajouter un nouvel ETF depuis ETF_LIBRARY
            st.markdown("---")
            ...
            if available:
                chosen = st.selectbox("ETF à ajouter", ["(choisir)"] + available,
                                      key="sidebar_add_etf")
                if chosen != "(choisir)" and st.button("➕ Ajouter", key="sidebar_add_btn"):
                    meta = ETF_LIBRARY[chosen]
                    new_raw.append({"ticker": chosen, "parts": 0.0,
                                    "prm": 0.0, "account": meta["enveloppe"]})
        ...
        # v5.5 : Expanders depuis st.session_state["positions"] (format enrichi)
        positions_conf = []
...
""", unsafe_allow_html=True)
    ...
    def render_transactions_tab(self):
        """Onglet d'enregistrement et de visualisation des ordres BUY/SELL."""
        ...
        with st.expander("➕ Enregistrer un nouvel ordre", expanded=True):
            ...
            with c3:
                tx_parts = st.number_input("Parts", min_value=0.0, value=0.0,
                                            format="%.4f", step=0.0001, key="tx_parts")
            with c4:
                ...
                tx_price  = st.number_input("Prix unitaire (€)", min_value=0.0,
                                             value=default_p, format="%.4f",
                                             step=0.01, key="tx_price")
            with c5:
            ...
            with col_btn:
                if st.button("✅ Enregistrer l'ordre", type="primary", use_container_width=True):
                    if tx_parts > 0 and tx_price > 0:
                        ...
                        if ok:
                            # Recalcul et synchronisation des positions
            ...
            with col_info:
                if tx_parts > 0 and tx_price > 0:
                    ...
                    st.markdown(
                        ...
                        f'Enveloppe : {ETF_LIBRARY.get(tx_ticker, {}).get("enveloppe", "N/A")}'
                        f'</div>', unsafe_allow_html=True)
        ...
        # ── Recap depuis TransactionEngine ──────────────────────────────────
...
""")
    # ── Rendu UI ──────────────────────────────────────────────────────────────
    # ── v5.5 : Tabs Dashboard + Transactions ────────────────────────────────
